    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
        ...
        <input
          ...
          className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-4 py-3 text-sm placeholder-gray-600 focus:outline-none focus:border-gray-500 transition-colors"
        />
      </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-10 text-center">
            <p className="text-gray-500 text-sm">{search ? 'No matching volunteers.' : 'No volunteers yet.'}</p>
          </div>
        ) : (
          filtered.map((v: any) => (
            <div
              key={v.student.id}
              className="bg-gray-900 border border-gray-800 rounded-2xl flex items-center gap-4 p-4"
            >
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gray-800 text-gray-300 text-sm font-medium hover:bg-gray-700 hover:text-white disabled:opacity-50 transition-colors shrink-0"
              >
