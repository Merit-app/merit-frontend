...
export function OrgCommunityCard({ stats, loading }: Props) {
  if (loading) {
    return (
      <div className="bg-card rounded-xl border border-border p-5">
        <h2 className="text-[15px] font-semibold text-foreground mb-4">Merit Community</h2>
  ...
  if (!stats || stats.totalStudents === 0) {
    return (
      <div className="bg-card rounded-xl border border-border p-5">
        <h2 className="text-[15px] font-semibold text-foreground mb-3">Merit Community</h2>
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-5">
      ...
      {/* Big stats row */}
      <div className="flex gap-4 justify-around mb-4 pb-4 border-b border-border">
      ...
      <div className="space-y-1.5 text-[13px] text-muted-foreground">
        {stats.mostActiveMonths.length > 0 && (
          <p>
            ...
            <span className="font-medium text-foreground">{stats.mostActiveMonths.join(', ')}</span>
          </p>
        )}
        <p>
          <span className="font-medium text-foreground">{stats.recentVolunteerCount}</span>{' '}
