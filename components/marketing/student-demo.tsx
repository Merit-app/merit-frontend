      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-10 lg:gap-16">
        ...
        <div className="relative flex flex-col items-center">
          ...
          <Phone>
            ...
            {/* Push notification — only on the "They reply YES" step */}
            <AnimatePresence>
              {s.studentScreen === 'verified' && <VerifiedPush key="push" />}
            </AnimatePresence>
          </Phone>
        </div>
...
function Phone({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative w-[200px] h-[420px] md:w-[230px] md:h-[480px] rounded-[3rem] shadow-2xl border overflow-hidden bg-card border-border">
      <div className="absolute inset-[8px] rounded-[2.5rem] overflow-hidden bg-card">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-24 h-6 rounded-b-2xl z-30 bg-card" />
...
// Slides down from the top of the phone, holds, then fades out — loops while the
// "They reply YES" step is active. The "ding, you're verified" payoff moment.
function VerifiedPush() {
  return (
    <motion.div
      className="absolute top-2 left-2 right-2 z-40"
      initial={{ y: -90, opacity: 0 }}
      animate={{ y: [-90, 6, 6, 6, -90], opacity: [0, 1, 1, 1, 0] }}
      transition={{
        duration: 4,
        times: [0, 0.13, 0.45, 0.85, 1],
        repeat: Infinity,
        repeatDelay: 0.4,
        ease: APPLE,
      }}
    >
      <div className="bg-card/95 backdrop-blur-md rounded-2xl shadow-xl border border-border px-3 py-2.5 flex items-start gap-2.5">
        <div className="w-7 h-7 rounded-lg bg-green-500 grid place-items-center shrink-0 shadow-sm">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-1.5">
            <div className="flex items-center gap-1.5">
              ...
              <p className="text-[11px] font-bold text-foreground">merit.</p>
            </div>
            <span className="text-[9px] text-muted-foreground">now</span>
          </div>
          <p className="text-[11px] text-foreground leading-snug mt-0.5">
            Your <span className="font-semibold">4 hours</span> at Vancouver Food Bank were verified! ✓
          </p>
        </div>
      </div>
    </motion.div>
  ...
  return (
    <ScreenFade>
      <div className="px-5 pt-2 pb-4 flex flex-col h-full">
        <h2 className="text-lg font-bold text-foreground mb-5">Log session</h2>
  ...
  return (
    <ScreenFade>
      <div className="px-5 pt-2">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ ...SP, delay: 0.1 }} className="bg-card border border-border rounded-2xl p-4 shadow-sm">
          <div className="flex items-start justify-between mb-3">
            <div>
              <p className="font-semibold text-foreground text-sm">Sorted food donations</p>
              <p className="text-xs text-muted-foreground mt-0.5">Vancouver Food Bank</p>
            </div>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="text-xs text-muted-foreground text-center mt-6 leading-relaxed">
...
function SVerifiedScreen() {
  return (
    <ScreenFade>
      <div className="px-5 pt-2">
        <h2 className="text-lg font-bold text-foreground mb-4">Sessions</h2>
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ ...SP, delay: 0.1 }} className="bg-card border border-green-200 rounded-2xl p-4 shadow-sm">
          <div className="flex items-start justify-between mb-3">
            <div>
              <p className="font-semibold text-foreground text-sm">Sorted food donations</p>
              <p className="text-xs text-muted-foreground mt-0.5">Vancouver Food Bank</p>
...
function SExportScreen() {
  return (
    <ScreenFade>
      <div className="px-5 pt-2 pb-4 flex flex-col h-full">
        <h2 className="text-lg font-bold text-foreground mb-4">Export PDF</h2>
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ ...SP, delay: 0.1 }} className="bg-muted border border-border rounded-2xl p-4 flex-1 flex flex-col">
          <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-widest">Merit Verified Report</p>
          <p className="font-bold text-foreground mt-1">Sarah Kim</p>
          <p className="text-xs text-muted-foreground">28 verified hours · 7 sessions</p>
          <div className="mt-4 space-y-2 flex-1">
            {[
              ...
              <div key={s.org} className="flex items-center gap-2 text-xs">
                ...
                <span className="flex-1 text-muted-foreground truncate">{s.org}</span>
                <span className="font-bold text-foreground">{s.hrs}</span>
              </div>
            ))}
          </div>
          <div className="flex items-center gap-2 mt-4 pt-3 border-t border-border">
            <div className="w-8 h-8 bg-card border border-border rounded-lg grid place-items-center text-[6px] font-mono text-muted-foreground">QR</div>
        ...
        <div className="space-y-3">
          {['Mom', 'Work Team', 'Sarah K.'].map((n, i) => (
            <motion.div key={n} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} transition={{ ...SP, delay: 0.1 + i * 0.05 }} className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-muted grid place-items-center text-sm font-bold text-muted-foreground shrink-0">{n[0]}</div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-foreground">{n}</p>
                <p className="text-xs text-muted-foreground truncate">{i === 0 ? 'Sounds good!' : i === 1 ? 'Meeting moved to 3pm' : 'Thanks!'}</p>
              </div>
              <span className="text-[10px] text-muted-foreground shrink-0">{i === 0 ? '2m' : i === 1 ? '1h' : 'Tue'}</span>
            </motion.div>
          ))}
        </div>
      </div>
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={SPB} className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <p className="text-sm font-bold text-foreground">Merit</p>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ ...SPB, delay: 0.2 }} className="bg-muted rounded-2xl rounded-tl-sm px-3 py-2.5 max-w-[200px]">
            ...
            <p className="text-[10px] text-muted-foreground mt-1">Merit · now</p>
          </div>
        </motion.div>
        {['Mom', 'Work Team'].map((n) => (
    ...
    <ScreenFade>
      <div className="px-5 pt-2">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <p className="font-bold text-foreground text-sm">Merit</p>
            <p className="text-[10px] text-muted-foreground">Verification</p>
          </div>
        </div>
        <div className="flex justify-start mb-3">
          <div className="bg-muted rounded-2xl rounded-tl-sm px-3 py-2.5 max-w-[75%]">
            <p className="text-sm text-foreground leading-snug">Verify Sarah Kim&apos;s 4 hours at Vancouver Food Bank? Reply YES to confirm.</p>
          </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ ...SPB, delay: 0.9 }} className="flex justify-start">
          <div className="bg-muted rounded-2xl rounded-tl-sm px-3 py-2.5 max-w-[75%]">
            <p className="text-sm text-foreground leading-snug">✓ Confirmed. Sarah Kim has been notified.</p>
          </div>
        </motion.div>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="text-center text-xs text-muted-foreground mt-6">
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={n} className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-full grid place-items-center text-sm font-bold shrink-0 ${n === 'Merit' ? 'bg-green-500 text-white' : 'bg-muted text-muted-foreground'}`}>{n[0]}</div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-foreground">{n}</p>
                <p className={`text-xs truncate ${n === 'Merit' ? 'text-green-600 font-medium' : 'text-muted-foreground'}`}>{sub}</p>
              </div>
            </div>
          ))}
        </div>
...
function InputRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wide mb-1.5">{label}</p>
      <div className="bg-muted rounded-xl px-3 py-2.5 text-sm text-foreground min-h-[2.5rem] flex items-center">{children}</div>
    </div>
  );
}
